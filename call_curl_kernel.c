...
int  MPI_Yee_FDTD_MUR_ABC (Field3D_MPI *  pthis ,Field3D_MPI *  inEB ,double  DT ,int  use_abc ,int  use_pec ,int  use_damp ,int  use_fixb ){
	Field3D_Seq *  data = 	( pthis )->data ;
	long  num_runtime = 	( pthis )->num_runtime ;
	PS_MPI_Comm  comm = 	( pthis )->comm ;
	long  cur_rank = 	( pthis )->cur_rank ;
	...
	if (  	(  use_fixb == 0 )  ){  
			Field3D_MPI_yee_setfix_ ( pthis , inEB , DT );

	}else{
			if (  	(  use_fixb == 1 )  ){  
			Field3D_MPI_yee_setfix_x ( pthis , inEB , DT );

	}else{
			if (  	(  use_fixb == 2 )  ){  
			Field3D_MPI_yee_setfix_y ( pthis , inEB , DT );

	}else{
			if (  	(  use_fixb == 3 )  ){  
			Field3D_MPI_yee_setfix_xy ( pthis , inEB , DT );

	}else{
			if (  	(  use_fixb == 4 )  ){  
			Field3D_MPI_yee_setfix_z ( pthis , inEB , DT );

	}else{
			if (  	(  use_fixb == 5 )  ){  
			Field3D_MPI_yee_setfix_xz ( pthis , inEB , DT );

	}else{
			if (  	(  use_fixb == 6 )  ){  
			Field3D_MPI_yee_setfix_yz ( pthis , inEB , DT );

	}else{
			if (  	(  use_fixb == 7 )  ){  
			Field3D_MPI_yee_setfix_xyz ( pthis , inEB , DT );
...
int  MPI_kgm_eqn_core (Field3D_MPI *  pthis ,Field3D_MPI *  inEB ,Field3D_MPI *  pextA0 ,Field3D_MPI *  pextA1 ,double  DT ,double  m ,double  Q ,double  DX ,double  extg ,double  refz0 ,double  g_beg ,int  swap_input ){
	Field3D_Seq *  data = 	( pthis )->data ;
	long  num_runtime = 	( pthis )->num_runtime ;
	PS_MPI_Comm  comm = 	( pthis )->comm ;
	long  cur_rank = 	( pthis )->cur_rank ;
	...
	int  num_spec = 	( inEB )->num_spec ;
	double  damp_vars = 	( inEB )->damp_vars ;
	Field3D_Seq_kgm_eqn_core ( 	(  data0 + i ) , 	(  data + i ) , 	(  	( pextA0 )->data + i ) , 	(  	( pextA1 )->data + i ) , DT , m , Q , DX , extg , refz0 , g_beg , swap_input );
}	return  0 ;}
