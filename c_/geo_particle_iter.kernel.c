...
void  c_geo_rel_1st_fwd_scmc_kernel (double *  inoutput ,int *  xyzw ,double *  cu_cache ,int *  cu_xyzw ,int *  xoffset ,int *  yoffset ,int *  zoffset ,double *  fieldE ,double *  fieldB ,double *  fieldB1 ,double *  FoutJ ,long  XLEN ,long  YLEN ,long  ZLEN ,int  ovlp ,long  numvec ,int  num_ele ,long  grid_cache_len ,long  cu_cache_length ,double  DELTA_X ,double  DELTA_Y ,double  DELTA_Z ,double  Mass0 ,double  Charge0 ,double  Deltat ,double  Tori_X0 ,double  Solve_Err ,long  scmc_internal_g_idy ,long  scmc_internal_g_ylen ){
	...
	int  Midx = 	c_floorx ( (	(  cu_cache + 	(  	(  idy * 	(  6 * cu_cache_length ) ) + 	(  l1 * 6 ) ) ))[0] ) ;
	int  Midy = 	c_floorx ( (	(  cu_cache + 	(  	(  idy * 	(  6 * cu_cache_length ) ) + 	(  l1 * 6 ) ) ))[1] ) ;
	int  Midz = 	c_floorx ( (	(  cu_cache + 	(  	(  idy * 	(  6 * cu_cache_length ) ) + 	(  l1 * 6 ) ) ))[2] ) ;
	int  Bidx = 	(  Midx - 2 ) ;
	int  Bidy = 	(  Midy - 2 ) ;
	int  Bidz = 	(  Midz - 2 ) ;
...
}}}}}	double   a_f  [1][3];0;
{
	...
	double  Midx0 = 	c_floorx ( xx0 ) ;
	double  Midy0 = 	c_floorx ( xy0 ) ;
	double  Midz0 = 	c_floorx ( xz0 ) ;
	double  Midx1 = 	c_floorx ( xx1 ) ;
	double  Midy1 = 	c_floorx ( xy1 ) ;
	double  Midz1 = 	c_floorx ( xz1 ) ;
	...
	{
{
	...
	double  xy2 = Y2 ;
	double  xz2 = Z2 ;
	int  Midx2 = 	c_floorx ( xx2 ) ;
	int  Midy2 = 	c_floorx ( xy2 ) ;
	int  Midz2 = 	c_floorx ( xz2 ) ;
...
((	(  FoutJ + 	(  	(  num_ele * 	(  idy * 	(  	(  XLEN + 	(  2 * ovlp ) ) * 	(  	(  YLEN + 	(  2 * ovlp ) ) * 	(  ZLEN + 	(  2 * ovlp ) ) ) ) ) ) + 	(  0 + 	(  num_ele * 	(  	(  Bidx + ovlp ) + 	(  	(  XLEN + 	(  2 * ovlp ) ) * 	(  	(  	(  xyzy + Bidy ) + ovlp ) + 	(  	(  YLEN + 	(  2 * ovlp ) ) * 	(  	(  xyzz + Bidz ) + ovlp ) ) ) ) ) ) ) ) ))[Gll_P9918] = (	(  shJ0 + total_idx ))[Gll_P9918]);
	...
	double  grid_geo_z = 	(  DELTA_Z / 	(  DELTA_X * DELTA_Y ) ) ;
	double  XO = (xoffset)[idy] ;
	int  Midx = 	c_floorx ( (	(  inoutput + 	(  grid_base_offset + 	(  6 * 	(  allgid * grid_cache_len ) ) ) ))[0] ) ;
	int  Midy = 	c_floorx ( (	(  inoutput + 	(  grid_base_offset + 	(  6 * 	(  allgid * grid_cache_len ) ) ) ))[1] ) ;
	int  Midz = 	c_floorx ( (	(  inoutput + 	(  grid_base_offset + 	(  6 * 	(  allgid * grid_cache_len ) ) ) ))[2] ) ;
	int  Bidx = 	(  Midx - 2 ) ;
	int  Bidy = 	(  Midy - 2 ) ;
	int  Bidz = 	(  Midz - 2 ) ;
	double  Mass = 1.00000000000000000e+00 ;
